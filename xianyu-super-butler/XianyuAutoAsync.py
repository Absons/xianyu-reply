...
class XianyuLive:
    ...
    async def handle_message(self, message_data, websocket):
        ...
        try:
            ...
            # 【自动发货触发补充】付款卡片更新消息（message['1']为字符串、无聊天结构）
            # 不会进入下方 is_chat_message 分支，但其 message['4'].reminderContent
            # 可能是"[已付款，待发货]"等付款完成信号，这里补一条触发路径，
            # 否则该类消息被当作"非聊天消息"丢弃，自动发货永远不会执行。
            try:
                if (
                    isinstance(message, dict)
                    and isinstance(message.get("4"), dict)
                    and isinstance(message.get("2"), str)
                ):
                    card_info = message["4"]
                    card_content = card_info.get("reminderContent", "")
                    if isinstance(card_content, str) and self._is_auto_delivery_trigger(card_content):
                        msg_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        chat_id = str(message.get("2", "")).split('@')[0]
                        send_user_id = str(card_info.get("senderUserId", "unknown"))
                        send_user_name = card_info.get("senderNick") or card_info.get("reminderTitle") or "未知用户"
                        item_id = "未知商品"
                        reminder_url = card_info.get("reminderUrl", "")
                        if isinstance(reminder_url, str) and reminder_url:
                            _m = re.search(r'itemId=(\d+)', reminder_url)
                            if _m:
                                item_id = _m.group(1)
                        logger.info(f'[{msg_time}] 【{self.cookie_id}】检测到卡片消息付款信号（{card_content}），进入订单校验')
                ...
                logger.error(f"【{self.cookie_id}】处理卡片付款信号失败: {self._safe_str(_card_e)}")
